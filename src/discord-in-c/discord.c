...
void internal_gateway_callback(SSL *ssl, void *state, char *msg,
  ...
  int ret = sm_get(discord->data_dictionary, DISCORD_GATEWAY_BOT_ID, bot_id,
                   DISCORD_MAX_ID_LEN);

  if (strcasestr(msg, DISCORD_GATEWAY_VOICE_STATE_UPDATE) && ret &&
      strstr(msg, bot_id)) {
...
void authenticate_gateway(discord_t *discord, char *discord_intent) {
  ...
  snprintf(msg, MAX_BOT_TOKEN_LEN + auth_str_len, DISCORD_GATEWAY_AUTH_STRING,
           bot_token, discord_intent);
...
void udp_hole_punch(voice_gateway_t *vgt) {
  ...
  int cnt = 0;
  cnt = recvfrom(fd, buffer, sizeof(buffer), 0, (struct sockaddr *)&src_addr,
                 &src_addr_len);
  for (int i = 0; i < cnt; i++) {
    printf("%x ", buffer[i]);
  }
